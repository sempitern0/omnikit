...
static func get_palette(id: StringName) -> OmniKitColorPalette:
	var regex = RegEx.new()
	regex.compile(".tres$")
	
	for color_palette_path: String in OmniKitFileHelper.get_files_recursive(ColorPalettesPath, regex):
		var color_palette: OmniKitColorPalette = ResourceLoader.load(color_palette_path, "", ResourceLoader.CACHE_MODE_REUSE)
...
static func get_gradient(id: StringName) -> OmniKitColorGradient:
	...
	for gradient_path: String in OmniKitFileHelper.get_files_recursive(GradientsPath, regex):
		var gradient: OmniKitColorGradient = ResourceLoader.load(gradient_path, "", ResourceLoader.CACHE_MODE_REUSE)
...
static func generate_random_hsv_colors(number_of_colors: int = 12, saturation: float = 0.5, value: float = 0.95) -> PackedColorArray:
	...
	for i: int in number_of_colors:
		var h = randf()
		h += OmniKitMathHelper.GoldenRatioConjugate
		h = fmod(h, 1.0)
