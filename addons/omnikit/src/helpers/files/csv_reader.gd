	...
	if as_dictionary and lines.size() > 1:
		...
		for i in range(1, lines.size()):
			...
			if current_fields.size() > column_headers.size():
				push_error("OmniKitCSVReader: Reading the file %s it was detected a line that has more fields than column headers [] < []" % [path, column_headers.join(","), current_fields.join(",")])
				return ERR_PARSE_ERROR
